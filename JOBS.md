...
- [x] Decide on custom domain vs. default `xlab.github.io`. Done 2026-08-11 — **moot as originally framed**: `xlab.github.io` was never available (the GitHub account `xlab` belongs to an unrelated third party). Decided to keep the repo name and deploy as a project page at `https://bhargava1424.github.io/xlab.github.io/` rather than rename the repo or buy a custom domain. See SPEC.md §2/§9.
- [x] Fix the site for the real deploy target. Done 2026-08-11 — added `basePath`/`assetPrefix` to `next.config.ts` (new `lib/base-path.ts`, single source of truth for the username/repo-name the URL depends on). In the process, found and fixed a real bug this surfaced: `next/image` doesn't auto-prepend `basePath` to local `src` under `images.unoptimized: true`, so every person photo / project thumbnail / publication thumbnail / logo (12 call sites across 10 files) was silently 404ing under the subpath — all now route through `withBasePath()`. Verified by exporting a fresh build and serving it from a local dir structured to mimic the real `<user>.github.io/<repo>/` layout: every page, asset, and JS chunk resolves 200 under `/xlab.github.io/...`, and the old unprefixed root path correctly 404s.
- [ ] Final full-site review against `SPEC.md` §6 — every field accounted for, nothing silently dropped. Not re-run this session beyond the basePath fix above — SPEC.md §6's own field-by-field mapping is current as of Phase 5 and nothing in this session's changes touched content placement, only deploy plumbing.
- [ ] Confirm GitHub Pages deployment is live and correct at the real domain. **Blocked on a manual step outside this environment**: no `gh` CLI / GitHub API token is available here, so Pages enablement status can't be confirmed or toggled directly — the live URL and the Pages API both currently 404, meaning Pages likely isn't enabled yet despite `deploy.yml` existing. Needs a human to check github.com/Bhargava1424/xlab.github.io/settings/pages (Source should be "GitHub Actions") after this session's fix is pushed, then confirm `https://bhargava1424.github.io/xlab.github.io/` actually loads the site.
