...
Designed against three constraints established while planning this (see `docs/PLAN.md`):
...
3. **"Mongo, not SQL": link liberally, enforce nothing.** Wherever a real relationship might exist between two entities, model it as an optional id reference — but never require it, never validate referential integrity at write time, and always let a record exist with the reference unset. A `Person` a record references doesn't have to exist yet; a `Publication` doesn't have to name any lab-member author at all. This is what makes the schema "always editable" the way the user asked for — content can be added incrementally, imported in bulk, and linked up later without a migration.

This third point is a **course-correction**, not the original design: `Publication.authors` first shipped as a plain string specifically to avoid partial-linking, and it was wrong — see below.
...
- **`Project`** — the poloclub-style card unit: title, tagline, thumbnail, optional links (paper/code/demo/video/poster), optional single `themeId`. This is expected to be the fastest-growing entity over time as more tools/demos exist. `collaborationWith` is a free-text **external org/company** name (matches poloclub's "Collaboration with X" badge, e.g. "Apple") — it is not, and was never meant to be, a `Person` link. `contributors?: string[]` is the actual `Person` link: which lab members worked on this project. Optional, populated only when known — leave it off rather than guess.
- **`Publication`** — one unified shape across all 6 categories (patent/journal/conference/workshop/invited-paper/book-chapter), discriminated by `category`, with category-specific optional fields rather than 6 parallel types. `id` is generated once at normalization and then treated as immutable, since other entities reference it. `authors` is an **ordered array of `{ name, personId? }`**, not a plain string — `name` is always the source of truth for display text/order (most entries, external co-authors, have no `personId`), but whichever authors *are* lab members get linked. This replaced an earlier plain-string design that avoided linking entirely on the theory that "most co-authors will never be lab members, so partial linking would look inconsistent" — that reasoning was backwards: partial linking (some names as links, some as plain text) is completely standard on real academic sites, and the earlier version meant that even the PI, who's on nearly every publication, never linked back to his own `Person` record. Fixed by linking exactly the authors we can confirm, nothing more.
